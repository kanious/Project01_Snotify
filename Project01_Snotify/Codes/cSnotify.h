...
// ¡å¡å¡å MyWork ¡å¡å¡å //
private:
	hash_map<unsigned int, cPerson*>		m_mapUsers;
	hash_map<unsigned int, cSong*>			m_mapSongs;
	SoundMaster*							m_pSoundMaster;

public:
	void PlaySong(cSong* pSong);
	void StopSong(cSong* pSong);
// ¡ã¡ã¡ã MyWork ¡ã¡ã¡ã //
};

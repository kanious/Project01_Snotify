...
// ¡å¡å¡å MyWork ¡å¡å¡å //
private:
	vector<cSong*> m_vecLibrary;
public:
	cPerson(cPerson* pPerson);
	void UpdateInformation(cPerson* pPerson);
	void AddSong(cSong* song);
	bool RemoveSong(unsigned int songID);
	bool UpdateRateOnSong(unsigned int songID, unsigned int newRating);
	bool GetSong(unsigned int songID, cSong*& song);
	bool GetRating(unsigned int songID, unsigned int& rating);
	bool GetNumberOfPlays(unsigned int songID, unsigned int &numberOfPlays);
	vector<cSong*>* GetLibrary() { return &m_vecLibrary; }
// ¡ã¡ã¡ã MyWork ¡ã¡ã¡ã //
};
